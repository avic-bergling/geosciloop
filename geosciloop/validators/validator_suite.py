...
from geosciloop.validators.model_diagnostics import (
    check_categorical_encoding_documented,
    check_model_metrics_exist,
    check_predictor_multicollinearity,
    check_risk_indicator_model_usage,
    check_spatial_split_strategy,
)
...
def run_validator_suite(
    ...
    results.extend(check_categorical_encoding_documented(config, data, evidence))
    results.extend(check_spatial_split_strategy(data, evidence))
    results.extend(check_risk_indicator_model_usage(config, evidence))
    results.extend(check_predictor_multicollinearity(config, data, evidence))
